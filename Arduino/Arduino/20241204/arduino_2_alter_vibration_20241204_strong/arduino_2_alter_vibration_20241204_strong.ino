...
void executeSingleVibration(char command)
{
  switch (command)
  {
    ...
    case 'r':
      ...
      break;
    case 'm':
      activateVibratorStrong(aPin, true);
      break;
    case 'n':
...
void rotateClockwise()
{
  ...
  {
    previousMillis = currentMillis; // Update last switch time

    // Stop current vibrator
    analogWrite(currentPin, 0);

    // Clockwise switch pin
    if (currentPin == wPin)
    {
      currentPin = dPin;
    }
    else if (currentPin == dPin)
    {
      currentPin = wPin;
    }

    // Activate new vibrator
    analogWrite(currentPin, 100);
  }
}
...
void rotateCounterclockwise()
{
  ...
  if (currentMillis - previousMillis >= interval)
  {
    ...
    // Counterclockwise switch pin
    if (currentPin == wPin)
    {
      currentPin = aPin;
    }
    else if (currentPin == aPin)
    {
      currentPin = wPin;
    }
...
{
  analogWrite(pin, state ? 100 : 0); // PWM signal
}

void activateVibratorStrong(int pin, bool state)
{
  analogWrite(pin, state ? 255 : 0); // PWM signal
}
